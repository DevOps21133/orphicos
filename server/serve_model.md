# OrphicOS Brain â€” Local Model Serving (living doc)
...
- Docker Desktop not installed â†’ Docker path not considered.
...
```powershell
# From Windows â€” launches the server in its own WSL window:
.\server\scripts\brain_up.ps1

# Health check:
.\server\scripts\brain_health.ps1
```
...
weights from Hugging Face (~16 GB, one-time â€” the only permitted network
activity besides dependency installs).

Serving parameters (see `server/scripts/vllm_serve.sh`):
- `--max-model-len 32768` â€” the engine needs >= 20k tokens of context.
- `--gpu-memory-utilization 0.85` â€” leaves VRAM headroom for the local STT
  model that will live alongside the brain (Phase 5).
- `--limit-mm-per-prompt {"image": 5}` â€” multiple screenshots per request.
